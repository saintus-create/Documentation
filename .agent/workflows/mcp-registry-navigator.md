...
curl -s "https://api.github.com/search/repositories?q=mcp-server+in:name&sort=stars&order=desc" | jq '.items[:10] | .[].full_name'
```

### 2. Registry Check
- Query mcp.so API for verified servers
- Cross-reference with npm/PyPI for package availability
- Check modelcontextprotocol/registry for official listings

### 3. Capability Extraction
For each discovered server:
- Parse `mcp.json` or `package.json` for tool definitions
- Identify transport methods (stdio, HTTP, SSE)
- Document required environment variables

### 4. Configuration Generation
Generate `.mcp.json` entries with:
